...
public class IS24ApiImpl implements Is24Api, InternalObjectApi {
	...
	private String uploadVideo(String username, String realEstateId, IMultimediaObject object) throws IOException {
		...
		if (StringUtils.isBlank(videoId)) {
			...
			// sleep(5000);
			// LOG.warn("Connection problems during video upload. Going for
			// retry "
			// + (++retryCount), e);
			//
	...
	public void publishRealestate(String realestateId, String publishChannelId) {
		String xml = "<common:publishObject xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns:common=\"http://rest.immobilienscout24.de/schema/common/1.0\">" + NL + "<realEstate id=\"" + realestateId
				+ "\"/><publishChannel id=\"" + publishChannelId + "\"/></common:publishObject>";
	...
	public InputStream getAttachmentData(StreamingVideo video) {
		try {
			...
			// "/search/v1.0/expose/62484060/video/" + id);
			// String xml = sendGetRequest(url);
			// System.out.println(xml);
